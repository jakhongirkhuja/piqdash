              ...
              <div class="form-group row">
                <div class="col-md-12">
                  ...
                  <select v-model="category_id" @change="checkCategory" class="form-control"
                    ...
                    <option v-for="category in categories" :key="category.id" v-bind:selected="category_id==category.id"  v-bind:value="category.id">
...
export default {
  ...
  methods: {
    ...
    async requestApi() {
      ...
      try {
        let token = localStorage.getItem('token');
        const response = await fetch(this.url + 'api/v1/course/editNew/'+this.$route.params.id, {
          method: 'POST',
          body: data,
          headers: {
        ...
        if (status == 401) {
          ...
          if (json.error) {
            ...
            // this.added = true;
            this.resSuccess = 'Course Successfully Edited';
          }
        }
    ...
    getCourse(){
      let newUrl = this.url+'api/v1/course?id='+this.$route.params.id;
           ...
           const options = {
            ...
            fetch(newUrl, options)
                ...
                .then((data) => {
                    ...
                    if(data.courseTypeByAccess==0 || data.access){
                    ...
                    this.price = data.coursePrice? data.coursePrice : 0;
