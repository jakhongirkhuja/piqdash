  ...
  <div class="mt-3">
    <div class="row">
      <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <h4 class="card-title">Edit Target Filter</h4>
  ...
  methods: {
    ...
    async requestApi() {
      ...
      try {
        ...
        const response = await fetch(this.url + 'api/v1/targetFilter/edit/'+this.$route.params.id, {
          method: 'POST',
          body: data,
          headers: {
        ...
        if (status == 401) {
          ...
          if (json.error) {
            ...
            this.resSuccess = 'Target filter Successfully Updated';
          }
        }
    ...
    requesttarget(){
        ...
        let newUrl = this.url+'api/v1/targetFilter?targetfilter_id='+this.$route.params.id;
          ...
          fetch(newUrl, options)
              ...
              .then((data) => {
                  ...
                  if(data.ageRange.includes(',')){
                    this.age = 1;
                    this.rage = data.ageRange.split(',');
                    
                  }else{
                    if(data.ageRange=='all'){
                      this.age = 'all';
                      
                    }else{
                      this.age = 0;
                      this.nage = data.ageRange;
                    }
                  }
                  console.log(data.ageRange.includes(','));  
                  this.groupID = data.group_id;
                  $('#mySelect2').val(['1', '2']);
                  this.gender = data.gender==null? 'all' : (data.gender==false? 0 : 1);

                  $('.js-example-basic-multiple').val(JSON.parse(data.roleList)).trigger('change');
              });
      },
    getGroups(){
