...
export default {
  ...
  methods: {
      ...
      if(imageName){
        ...
        if (result) {
          ...
          try {
            ...
            const response = await fetch(this.url + 'api/v1/store/physicalstore/products/image/delete/'+this.$route.params.id, {
              method: 'POST',
              body: data,
              headers: {
    ...
    async requestApi() {
      ...
      try {
        let token = localStorage.getItem('token');
        const response = await fetch(this.url + 'api/v1/store/physicalstore/products/edit/'+this.$route.params.id, {
          method: 'POST',
          body: data,
          headers: {
            'Accept-Language': 'en-US,en;q=0.8',
        ...
        if (status == 401) {
          ...
          if (json.error) {
            ...
            this.resSuccess = 'Physical Store Product Successfully Updated';
          }
        }
    ...
    getStore(){
      let newUrl = this.url+'api/v1/store/physicalstore/products?product_id='+this.$route.params.id;
           ...
           const options = {
            ...
            fetch(newUrl, options)
                ...
                .then((data) => {
                    if(data){
                      this.name_ru = JSON.parse(data.name).ru;
                      this.name_uz =JSON.parse(data.name).uz;
                      this.descriptionText_ru = JSON.parse(data.description).ru;
                      this.descriptionText_uz =JSON.parse(data.description).uz;
                      this.price = data.price;
                      this.amount = data.amount;
                     
                        try {
                          
                          JSON.parse(`${data.img}`).forEach(element => {
                          this.banner_ru_prev.push(this.url+'files/physicalStoreProduct/'+element)
                          // console.log( JSON.parse(`${data.productIMG}`).isArray);
                        });
                        } catch (error) {
                        //  console.log(error);
                          this.banner_ru_prev =  this.url+'files/physicalStoreProduct/'+data.img;
                        }
