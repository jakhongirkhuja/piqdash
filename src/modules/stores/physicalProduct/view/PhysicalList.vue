...
<template>
    <div class="row">
        <div class="col-md-3">
            ...
            <CrudButton  v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" urlName="physicalProduct.add" :btnName="'Add Product'"  />
        </div>
        <div class="col-md-6">
                      ...
                      <div class="table-responsive">
                        ...
                        <tbody>
                            <tr v-for="store in stores" :key="store.id">
                                ...
                                <td><img width="20" v-for="img in store.img.replace('[','').replace(']','').split(',')" :src="showImageUrl(img)" alt=""></td>
                                <td> {{$i18n.locale=='ru'? JSON.parse(store.name).ru :JSON.parse(store.name).uz }}</td>
                                <td>{{store.price}} $</td>
                                <td>{{store.amount}}</td>
                                ...
                                <td class="actions">
                                     ...
                                     <div  data-toggle="modal" data-target="#exampleModal cursor-pointer" v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" @click="openmodel(store.id, JSON.parse(store.name).ru)" class="delete text-danger" ><i class="ti ti-trash" data-toggle="tooltip" title="Edit"></i></div>
                                  </td>
...
export default {
    data() { 
        return {
            imageUrl : 'https://api.pharmiq.uz/files/physicalStoreProduct/',
            disabled: false,
            modelName:'',
            modelUserId:'',
            currentPage: 1,
    ...
    methods:{
       showImageUrl(url){
        ...
        showstore(store_id){
          this.$router.push({ name: 'physicalProduct.show', params: {'id': store_id}})
        },
        ...
        editstore(store_id){
            this.$router.push({ name: 'physicalProduct.edit', params: {'id': store_id}})
        },
        ...
        async requestApiDelete(){
            ...
            try {
              ...
              const response = await fetch(this.url+'api/v1/store/physicalstore/products/delete/'+this.modelUserId, {
              method: 'POST', // или 'PUT'
        ...
        requestApi(page, search=null){
          ...
          let newUrl = this.url+'api/v1/store/physicalstore/products?page='+page;
