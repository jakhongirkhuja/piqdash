          ...
          <div class="card-body">
            ...
            <form class="forms-sample" @submit.prevent="submitForm()">
              ...
              <div class="form-group row">
                ...
                <div class="col-md-12" v-if="importType==1">
                  <label class="col-sm-4" for="banner">Upload file</label>
                  
                  <input type="file" v-on:change="uploadFilebannerRu($event)" id="banner" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
...
export default {
  ...
  validations() {
    return {
      ...
      banner_ru:{
        required: requiredIf(() => { return this.importType == 1; }),
      }
    }
  },
  ...
  methods: {
    async requestApi() {
      ...
      if(this.importType==1){
        data.append('excel', this.banner_ru);
      }
      ...
      try {
        ...
        const response = await fetch(this.url + 'api/v1/store/korzinkabarcode/add', {
          method: 'POST',
          body: data,
          headers: {
        ...
        if (status == 401) {
          ...
          if (json.error) {
            ...
            this.resSuccess = 'Barcode Successfully Added';
          }
        }
