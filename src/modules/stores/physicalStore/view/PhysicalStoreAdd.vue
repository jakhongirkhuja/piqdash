          ...
          <div class="card-body">
            ...
            <form class="forms-sample" @submit.prevent="submitForm()">
              ...
              <div class="form-group row">
                ...
                <div class="col-md-6">
                  <label for="exampleInputMainContact">Main Contact</label>
                    <input type="text" class="form-control" placeholder="998991112233"
                      v-bind:class="{ 'border border-danger': v$.mainContact.$errors.length > 0 }" id="exampleInputMainContact"
                      v-model="mainContact">
                    <small class="error" v-for="(error, index) of v$.mainContact.$errors" :key="index">{{ error.$message
                    }}</small>
                </div>
                <div class="col-md-6">
                  <label for="exampleInputExtraContact">Extra Contact</label>
                    <input type="text" class="form-control" placeholder="998991112233"
                      v-bind:class="{ 'border border-danger': v$.extraContact.$errors.length > 0 }" id="exampleInputExtraContact"
                      v-model="extraContact">
                    <small class="error" v-for="(error, index) of v$.extraContact.$errors" :key="index">{{ error.$message
                    }}</small>
                </div>
              </div>
  ...
  methods: {
    async requestApi() {
      ...
      const inputs = {
        'latitude': this.lat,
        'longitude': this.long,
        'landmark_ru': this.storeLandmark_ru,
        'landmark_uz': this.storeLandmark_uz,
        'description_ru': this.descriptionText_ru,
        'description_uz': this.descriptionText_uz,
        'name': this.name,
        'category_id':this.categoryID,
        'owner': this.storeOwnerID,
      ...
      data.append('img', this.banner_ru)
      ...
      try {
        ...
        const response = await fetch(this.url + 'api/v1/store/physicalstore/add', {
          method: 'POST',
          body: data,
          headers: {
    ...
    getAddressByCoor(lat,long){
          ...
          fetch(newUrl, options)
              ...
              .then((data) => {
                  ...
                  console.log(this.storeLandmark_ru);
              });
    },
    submitForm() {
      this.v$.$touch();
