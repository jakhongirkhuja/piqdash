...
<template>
    <div class="row">
        <div class="col-md-3">
            ...
            <CrudButton  v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" urlName="physicalStore.add" :btnName="'Add Store'"  />
        </div>
        <div class="col-md-6">
                      ...
                      <div class="table-responsive">
                        ...
                        <tbody>
                            <tr v-for="market in markets" :key="market.id">
                                ...
                                <td> {{market.name}}</td>
                                <td>{{$i18n.locale=='ru'? JSON.parse(market.description).ru : JSON.parse(market.description).uz }}</td>
                                <td class="actions">
                                     ...
                                     <div  data-toggle="modal" data-target="#exampleModal cursor-pointer" v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" @click="openmodel(market.id, market.storeName)" class="delete text-danger" ><i class="ti ti-trash" data-toggle="tooltip" title="Edit"></i></div>
                                  </td>
...
export default {
    ...
    methods:{
       showImageUrl(url){
        ...
        editstore(store_id){
            this.$router.push({ name: 'physicalStore.edit', params: {'id': store_id}})
        },
        ...
        async requestApiDelete(){
            ...
            try {
              ...
              const response = await fetch(this.url+'api/v1/store/physicalstore/delete/'+this.modelUserId, {
              method: 'POST', // или 'PUT'
        ...
        requestApi(page, search=null){
          ...
          let newUrl = this.url+'api/v1/store/physicalstore?page='+page;
