...
<template>
    <div class="row">
        <div class="col-md-3">
            ...
            <CrudButton  v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" urlName="news.add" :btnName="'Add News'"  />
        </div>
        <div class="col-md-6">
            <!-- <input type="search" @input="searchlesson" v-model="newsearch" class="form-control" :placeholder="$t('search')+''"> -->
        </div>
        ...
        <div class="row">
                    <div class="col-12">
                      <div class="table-responsive">
                        ...
                        <tbody>
                            <tr v-for="newseach in news" :key="newseach.id">
                              <td >{{$i18n.locale=='ru'? JSON.parse(newseach.title).ru :JSON.parse(newseach.title).uz}}</td>
                              <td ><img :src="showImageUrl(newseach.banner)"></td>
                                ...
                                <td class="actions">
                                     ...
                                     <div  @click="editlesson(newseach.id)" class="edit mr-2 cursor-pointer" v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" ><i class="ti ti-pencil-alt" data-toggle="tooltip" title="Edit"></i></div>
                                     <div  data-toggle="modal" data-target="#exampleModal cursor-pointer" v-if="userInfo && (userInfo.role=='Creator' || userInfo.role=='Owner')" @click="openmodel(newseach.id, JSON.parse(newseach.title).ru)" class="delete text-danger" ><i class="ti ti-trash" data-toggle="tooltip" title="Edit"></i></div>
                                  </td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body text-center">
        ...
        <p>News Id: {{modelUserId}}</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button v-if="submitButton" :disabled="disabled" type="button" @click="requestApiDelete" class="btn btn-primary">Delete news</button>
      </div>
    </div>
  </div>
</div>
...
export default {
    data() { 
        return {
            imageUrl : 'https://api.pharmiq.uz/files/news/',
            disabled: false,
            modelName:'',
            modelUserId:'',
            currentPage: 1,
    ...
    methods:{
        ...
        searchlesson(){
          ...
          if(this.newsearch!=''){
            this.loading = true;
              this.timerId = setTimeout(() =>{ this.requestApi(1,this.newsearch)}, 2000);
          }else{
            this.loading = true;
        ...
        showlesson(lesson_id){
          this.$router.push({ name: 'news.show', params: {'id': lesson_id}})
        },
        ...
        editlesson(lesson_id){
            this.$router.push({ name: 'news.edit', params: {'id': lesson_id}})
        },
        ...
        async requestApiDelete(){
            ...
            try {
              ...
              const response = await fetch(this.url+'api/v1/news/delete/'+this.modelUserId, {
              method: 'POST', // или 'PUT'
        ...
        requestApi(page, search=null){
          ...
          let newUrl = this.url+'api/v1-1/spa-news?page='+page;
