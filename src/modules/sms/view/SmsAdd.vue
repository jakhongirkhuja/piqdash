<template>
  <button 
    ...
    @click="$router.replace({ name: 'sms.control.all' })">&laquo; 
    Back
    </button>
  <div class="mt-3">
    <div class="row">
            ...
            <form class="forms-sample" @submit.prevent="submitForm()">
              ...
              <div class="form-group row">
                <div class="col-md-12">
                  ...
                  <div class="noError" v-if="resGenerateNoErrorGenerate">{{resGenerateNoErrorGenerate}}</div>
                </div>
                <div class="col-md-6">
                  <label for="">Url</label>
                ...
                <div class="col-md-6">
                  <label for="">Generated link</label> <span @click="copyToClipboard" v-if="generateLinkCopy" style="    display: inline-block;  background-color: #9a9797; padding: 2px 10px; border-radius: 6px; color: white;">
                    {{generateLinkCopy}}</span> <span v-if="generateLinkCopy" @click="copyToClipboard" style="text-decoration: underline; cursor: pointer;">{{ copied }} </span>
...
export default {
  ...
  data() {
    return {
      ...
      resGenerateNoErrorGenerate: null,
      generateLinkClickName : 'Generate',
      price: 0,
      bitlink_id:null,
      category_id: 1,
      sendby: 'SMS to phone Number',
      long_url:'https://downloadapp.pharmiq.uz?utm_source=',
    }
  },
  computed: {
    isDisabled: function () {
  ...
  methods: {
    copyToClipboard(){
      ...
      temp.type = "text"
      temp.value = this.generateLinkCopy;
    ...
    async updateLinkClick(){
        ...
        try {
          ...
          if (status == 200 || status == 201) {
            ...
            this.resGenerateErrorGenerate = null;
            this.resGenerateNoErrorGenerate = 'Updated';
          } else {
            this.resGenerateErrorGenerate = json.description;
            this.resGenerateNoErrorGenerate = null;
          }
      ...
      if(this.long_url.match(urlRE)){
        ...
        try {
          ...
          if (status == 200 || status == 201) {
            ...
            this.resGenerateNoErrorGenerate = 'Success';
          } else {
            this.resGenerateErrorGenerate = json.description;
          }
      ...
      try {
        ...
        if (status == 401) {
          ...
          if (json.error) {
            ...
            // this.added = true;
            this.resSuccess = 'Sms Successfully Added';
          }
        }
